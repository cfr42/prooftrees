% $Id: tag-tableau-e1.tex 11140 2025-08-24 16:36:48Z cfrees $
\DocumentMetadata{
 tagging=on,
 lang=en-GB,
 pdfversion=2.0,
 pdfstandard=ua-2,
 uncompress,
}
\tagpdfsetup{
  math/mathml/structelem,
}
\documentclass{article}
\usepackage[tableaux]{prooftrees}
\usepackage{turnstile}
\usepackage{unicode-math}
\makeatletter
\newbox\prooftrees@tableau@box
\newcount\prooftrees@tableau@id
\newtoks\prooftrees@tableau@toks
\ExplSyntaxOn
\dim_new:N \l__tableau_wd_dim
\dim_new:N \l__tableau_tht_dim
\tl_new:N \l__tableau_alt_tl
\cs_new_eq:NN \__tableau_property_ref_orig:nn \property_ref:nn
\cs_generate_variant:Nn \__tableau_property_ref_orig:nn {ee}
\cs_new_nopar:Npn \__tableau_pgftikz_tag_bbox:n #1
{
  \__tableau_pgftikz_tag_bbox_aux:eeVV
  {
    \__tableau_property_ref_orig:ee {#1}{xpos}
  }
  {
    \__tableau_property_ref_orig:ee {#1}{ypos}
  }
  \l__tableau_wd_dim
  \l__tableau_tht_dim
}
\cs_generate_variant:Nn \__tableau_pgftikz_tag_bbox:n {e}
\cs_new_nopar:Npn \__tableau_pgftikz_tag_bbox_aux:nnnn #1#2#3#4
{
  \dim_to_decimal_in_bp:n {#1sp}
  \c_space_tl
  \dim_to_decimal_in_bp:n {#2sp}
  \c_space_tl
  \dim_to_decimal_in_bp:n {#1sp+#3}
  \c_space_tl
  \dim_to_decimal_in_bp:n {#2sp+#4}
}
\cs_generate_variant:Nn \__tableau_pgftikz_tag_bbox_aux:nnnn {eeVV}
\socket_new_plug:nnn {tagsupport/tikz/picture/begin}{tableau/alt}
{
  \tag_mc_end_push:
  \tag_struct_begin:n
  {
    tag=Figure,
    alt=\l__tableau_alt_tl
  }
  \tag_mc_begin:n{tag=Figure}
}
\socket_new_plug:nnn {tagsupport/tikz/picture/end}{tableau/alt}
{
  \tag_mc_end:
  \cs_new:cpe {prooftrees@tableau@mark@pos@\the\prooftrees@tableau@id}
  {
    \__tableau_pgftikz_tag_bbox:e {prooftrees-tableau-id\the\prooftrees@tableau@id}
  }
  \tag_struct_gput:ene
  {\tag_get:n {struct_num}}
  {attribute}
  {
    /O /Layout /BBox~
    [
      \use:c
      {prooftrees@tableau@mark@pos@\the\prooftrees@tableau@id}
    ]
  }
  \tag_struct_end:
  \tag_mc_begin_pop:n{}
}
\cs_new_nopar:Npn \__tableau_ttableau_begin:
{
  \tag_if_active:T{
    \forestset{tag=1}
    \global\prooftrees@tableau@toks{}
    \socket_assign_plug:nn {tagsupport/tikz/picture/begin}{artifact}
    \socket_assign_plug:nn {tagsupport/tikz/picture/end}{artifact}
    \def\pgfsys@begin@text{}
    \def\pgfsys@end@text{}
    \tag_suspend:n{\tableau}
  }{\forestset{tag=0}}
}
\cs_new_eq:NN \prooftrees@ttableau@begin \__tableau_ttableau_begin:
\cs_new_nopar:Npn \__tableau_ttableau_end:nn #1#2
{
  \tag_resume:n {\tableau}
  \tl_set:No \l__tableau_alt_tl {\the\prooftrees@tableau@toks}
  \socket_assign_plug:nn {tagsupport/tikz/picture/begin}{tableau/alt}
  \socket_assign_plug:nn {tagsupport/tikz/picture/end}{tableau/alt}
  \dim_set:Nn \l__tableau_wd_dim {#1}
  \dim_set:Nn \l__tableau_tht_dim {#2}
  \tex_savepos:D
  \property_record:ee {prooftrees-tableau-id\the\prooftrees@tableau@id}
  {xpos,ypos}
  \tex_savepos:D
  \socket_use:n {tagsupport/tikz/picture/begin}
  \tag_suspend:n {\tableau}
  \box\prooftrees@tableau@box
  \tag_resume:n {\tableau}
  \socket_use:n {tagsupport/tikz/picture/end}
}
\cs_new_eq:NN \prooftrees@ttableau@end \__tableau_ttableau_end:nn
\ExplSyntaxOff
\forestset{%
  declare boolean register={tag},
  tag=0,
  declare dimen register={tableau ht plus dp},
  tableau ht plus dp'=0pt,
  declare dimen register={tableau wd},
  tableau wd'=0pt,
  declare keylist={tableau structure}{},
  declare keylist={tableau tag}{},
  declare toks register={tag close with},
  tag close with={closed},
  declare toks={ttoks}{},
  toksapp/.style={%
    TeX={%
      \toksapp\prooftrees@tableau@toks{#1}%
    },
  },
  ttableau/.style={%
    if={>R{tag}}{%
      % would really like to avoid this requirement
      draw tree box=\prooftrees@tableau@box,
      tableau structure={%
        for name={proof statement}{%
          if content={}{}{%
            toksapp={To prove:\ },
            toksapp/.process={Ow{content}{\ensuremath{##1}}},
          },
        },
        for unique={proof tree wffs}{%
          if proof tree rhifo={
            if line numbering={%
              ttoks+/.option=proof tree proof line no,
              ttoks+={\ },
            }{},
            ttoks+/.process={Ow{content}{\ensuremath{##1}}},
            if justifications={%
              ttoks+/.process={Ow+pw {proof tree proof line no}{O{!{name=just ##1}.content}}{\ ##1\ }},
            }{},
          }{%
            if proof tree crefs={}{}{%
              !uu.ttoks+/.process={ORw2{content}{tag close with}{\ ##2 ##1\ }},
            },
          },
        },
        for unique={name=proof statement,proof tree wffs}{%
          toksapp/.option=ttoks,
          toksapp={\ },
        },
      },
      tableau tag={%
        tempdima/.max={>OOw2+d{x}{max x}{##1+##2}}{tree},
        tempdima-/.min={>OOw2+d{x}{min x}{##1+##2}}{tree},
        tempdimb/.max={>OOw2+d{y}{max y}{##1+##2}}{tree},
        tempdimb-/.min={>OOw2+d{y}{min y}{##1+##2}}{tree},
        TeX/.process={%
          RRw2{tempdima}{tempdimb}{\prooftrees@ttableau@end{##1}{##2}}%
        },
      },
    }{},
  },
}
\bracketset{action character=@}
\NewDocumentEnvironment{ttableau}{ m +b }{% 
  \global\advance\prooftrees@tableau@id by 1
  \prooftrees@ttableau@begin
  \forest
    (%
      stages={% 
        for root'={% 
          process keylist register=default preamble,
          process keylist register=preamble,
        },
        process keylist=given options,
        process keylist=before typesetting nodes,
        process keylist=proof tree ffurf,
        process keylist=proof tree symud awto,
        typeset nodes stage,
        process keylist=before packing,
        pack stage,
        process keylist=before computing xy,
        compute xy stage,
        process keylist=proof tree creu nodiadau,
        process keylist=proof tree nodiadau,
        process keylist=before drawing tree,
        process keylist=tableau structure,
        draw tree stage,
        process keylist=tableau tag,
      },
    )%
    proof tree,
    ttableau,
    #1,
    [, name=proof statement @#2]%
  \endforest
}{}
\makeatother
\newcommand*{\lif}{\ensuremath{\mathbin{\rightarrow}}}
\title{t}
\begin{document}
\begin{ttableau}{%
  to prove=\sststile{}{}p\vee\lnot p,
}
  [\lnot(p\vee\lnot p),just=$\lnot$ conc
    [\lnot p,just=$\vee$E:!u,
      [\lnot\lnot p,just=$\vee$E:!uu,close={:!u,!c},]
    ]
  ]
\end{ttableau}
\begin{ttableau}
  {
    to prove={\{P \vee (Q \vee \lnot R), P \lif
      \lnot R, Q \lif \lnot R\} \sststile{}{} \lnot
    R}
  }
  [P \vee (Q \vee \lnot R), just=Ass, checked
    [P \lif \lnot R, just=Ass, checked
      [Q \lif \lnot R, just=Ass, checked, name=last premise
        [\lnot\lnot R, just={$\lnot$ Conc},
          name=not conc
          [P, just={$\vee$ Elim:!uuuu}
            [\lnot P, close={:!u,!c}]
            [\lnot R, close={:not conc,!c},
          just={$\lif$ Elim:!uuuu}]]
          [Q \vee \lnot R
            [Q, move by=1
              [\lnot Q, close={:!u,!c}]
              [\lnot R, close={:not conc,!c},
            just={$\lif$ Elim:last premise}]]
            [\lnot R, close={:not conc,!c},
  move by=1, just={$\vee$ Elim:!u}]]]]]]
\end{ttableau}
\end{document}


